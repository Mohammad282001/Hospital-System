...
function render(event) {
  ...
  let bod = document.getElementById("dob").value.trim();
  let gender = document.getElementById("gender").value.trim();
  let phone = document.getElementById("phone").value.trim();
  let chronicDiseases = document
  ...
  if (!validatePassword(password)) {
    alert(
      "Password must be more than 8 characters long and include at least one number, uppercase letter, and special character."
    );
    return;
  }
  ...
  displayPatient(patient); // Optional: Display the new patient immediately
}
...
function validateUsername(username) {
  return !/\s/.test(username); // returns true if there are no spaces
}
...
function isUserExists(username) {
  return (
    readPatientsArr &&
    readPatientsArr.some((patient) => patient.fullName === username)
  );
}
